...
@Component
public class ShuffleOnCommand implements SlashCommand {
    ...
    public Mono<Void> handle(ChatInputInteractionEvent event) {
        ...
        if (audioTracks.isPresent()) {
            if (!audioTrackScheduler.isShuffle()) {
                audioTrackScheduler.setShuffle(true);
                event.reply()
                        .withEphemeral(false)
                        .withEmbeds(messageBuilder.getBlankBuilder("Shuffle On"))
                        .subscribe();
            } else {
                audioTrackScheduler.setShuffle(false);
                event.reply()
                        .withEphemeral(false)
                        .withEmbeds(messageBuilder.getBlankBuilder("Shuffle Off"))
                        .subscribe();
            }
        } else {
            event.reply()
